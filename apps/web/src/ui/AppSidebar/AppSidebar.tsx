...
const itemClass = (active: boolean) =>
  [
    ...
    active ? 'bg-slate-700 text-white' : 'text-slate-400 hover:bg-slate-700/60 hover:text-white',
  ].join(' ');

const SidebarLink = ({ to, icon, label, exact = false }: NavItem & { exact?: boolean }) => (
  <NavLink to={to} end={exact} title={label} className={({ isActive }) => itemClass(isActive)}>
    {icon}
  </NavLink>
);
...
const AppSidebar = ({ open }: AppSidebarProps) => {
  ...
  return (
    <nav
      className={[
        'flex flex-col items-center justify-between w-16 bg-slate-800 border-r border-slate-700 py-4',
    ...
    >
      ...
      <div className="flex flex-col items-center gap-4">
        ...
        <SidebarLink to="/admin" icon={<Settings className="w-6 h-6" />} label="Einstellungen" />
      </div>
    </nav>
  );
};
